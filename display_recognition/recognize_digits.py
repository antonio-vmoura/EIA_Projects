...
class RecognizeDigits:
    ...
    def apply_threshold(image):
        warped_with_brightness, minimum_gray, maximum_gray = RecognizeDigits.brightness_and_contrast(image)
    ...
    def recognize_display(path_image, qty_decimal_digit=2, source="base64"):

        #Fazendo o load da imagem
        if source == "base64":
            image = RecognizeDigits.base64_to_image(path_image)
        elif source == "s3":
            image = RecognizeDigits.s3_to_image(path_image)
        else:
            image = cv2.imread(path_image)

        image = imutils.resize(image, height=500) #Aplicando um redicionamento para deixar a imagem plana

        displayCnt = RecognizeDigits.display_outline_coordinates(image)
        ...
        thresh = RecognizeDigits.apply_threshold(output) #Aplicando o threshold
        cv2.imwrite("result/apply_threshold.jpg", thresh) #salvando a imagem para visualizar o threshold

        digits = RecognizeDigits.check_numbers_on_display(thresh, output)

        print(f"digits {digits}")

        final_result = RecognizeDigits.final_result(digits, qty_decimal_digit)

        return final_result

digits = RecognizeDigits.recognize_display("./base64.txt")
print(f"\n\nValor do display: {digits}\n")